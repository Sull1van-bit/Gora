...
export default function PlotDetailHeader({ plot, onBack }) {
  return (
    <section className="bg-[#578a3e] rounded-b-[30px] pt-8 pb-5 px-4 relative text-white shadow-xs">
      
      <div className="flex items-center justify-between relative z-10 py-1">
        
        <button
          onClick={onBack}
          className="w-9 h-9 -mt-6 rounded-full flex items-center justify-center hover:bg-white/10 active:scale-90 transition-all"
          aria-label="Kembali"
        >
          <img src="/assets/figma/detail/back_button.svg" className="-rotate-90 w-6 h-6 filter brightness-0 invert shrink-0" alt="Back" />
        </button>
