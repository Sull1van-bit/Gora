...
export default function WeatherDetailView({ weather, hourlyForecast, weeklyForecast, locationLabel }) {
  ...
  const fallbackWeekly = [
    { day: 'Hari Ini', temp: '24° - 30°C', icon: '🌤️', cond: 'Cerah Berawan', rain: '25%' },
    { day: 'Besok', temp: '23° - 29°C', icon: '🌧️', cond: 'Hujan Ringan Sore', rain: '65%' },
    { day: 'Lusa', temp: '24° - 31°C', icon: '☀️', cond: 'Cerah Terik', rain: '10%' },
    { day: 'Jumat', temp: '24° - 30°C', icon: '🌤️', cond: 'Berawan Sebagian', rain: '20%' },
    { day: 'Sabtu', temp: '23° - 28°C', icon: '⛈️', cond: 'Hujan Petir Siang', rain: '80%' },
    { day: 'Minggu', temp: '24° - 29°C', icon: '🌤️', cond: 'Berawan', rain: '30%' },
  ];

  const hourlyToShow = hourlyForecast && hourlyForecast.length > 0 ? hourlyForecast : fallbackHourly;
  const weeklyToShow = weeklyForecast && weeklyForecast.length > 0 ? weeklyForecast : fallbackWeekly;
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-blue-600 via-indigo-700 to-slate-900 text-white rounded-3xl p-5 shadow-lg relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            <span className="bg-white/20 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-blue-100">
          ...
          <span className="text-5xl shrink-0">{weather?.icon || '🌤️'}</span>
        </div>
          ...
          {weeklyToShow.map((w, idx) => (
            <div key={idx} className="py-2.5 flex items-center justify-between text-xs">
              ...
              <div className="w-1/3 text-right">
                ...
                <span className="block text-[10px] text-blue-500 font-semibold">🌧️ {w.rain}</span>
              </div>
            </div>
          ))}
        </div>
